...
{{ img(src="https://zed.dev/img/post/zed-decoded-rope-sumtree/rope.png" info="A rope representing `This is a rope`" scale=0.4) }}
...
以下是匹配的类型定义, 你能在 [sum_tree](https://github.com/zed-industries/zed/blob/6721c91ab000cea73ab30209c4a57bd1e2e2ce56/crates/sum_tree/src/sum_tree.rs) 这个 crate 中找到它们:

{% code(path="crates/sum_tree/src/sum_tree.rs") %}
```rust
...
{{ img(src="https://zed.dev/img/post/zed-decoded-rope-sumtree/sumtree_diagram.png" info="A SumTree representing `Hello World!\nThis is\nyour captain speaking.\nAre you\nready for take-off?\n` with some summary fields left out" scale=1) }}
...
{{ img(src="https://zed.dev/img/post/zed-decoded-rope-sumtree/sumtree_three_lines.png" info="A SumTree representing `Line one.\nThis is the second line.\nThis is three.\n` with some summary fields left out" scale=1) }}
...
{% code(info="TextSummary & ChunkSummary") %}
```rust
struct TextSummary {
    len: usize,
    lines: Point,
...
{% code(info="point_to_point_utf16") %}
```rust
