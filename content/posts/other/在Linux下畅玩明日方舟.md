...
{% code(path="/etc/docker/daemon.json" info="在该文件添加如下内容") %}
```json
{
    "registry-mirrors": [
...
因为是伟大的 图像识别 技术, 你不需要担心会像某些模拟器的录制脚本一不小心把你的合成玉/源石全部花光, 而且具有绝对的操作精准度  
...
{% code(path="~/.config/hypr/keybindings.conf") %}
```rust
# Paths
...
{% code(path="~/.config/hypr/scripts/ydotool.sh") %}
```bash
#!/usr/bin/env bash
...
{% code(path="~/.config/hypr/startup.conf") %}
```bash
