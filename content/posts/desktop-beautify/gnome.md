...
下面是 bash 脚本, 传入该目录的路径, 自动进行安装:  

{% code(path="install-extension.sh" info="执行以下载传入目录中的所有插件") %}
```bash
#!/usr/bin/env bash
declare -a UUID_LIST
...
{% code(info="通过 `gnome-extensions` 来查看并配置插件") %}
```bash
# 获取帮助, `Command` 为可选项
...
{% code(info="通过 `gsettings` 配置插件, 但比较麻烦") %}
```bash
# 查看某个插件的所有选项
...
详情请见 [**加载配置**](#jia-zai-pei-zhi)(当你读到这里时大概率早就过时了)  
...
{% code(info="设置登陆 GDM 时的显示头像, 假设用户名是 `Xyz`") %}
```bash
...
{% code(info="主题: 单用户放 `~/.themes/`, 多用户放 `/usr/share/themes/`, 可前往 [gnome-look](https://www.gnome-look.org/) 挑选") %}
```bash
# 以 Nordic 为例子
...
{% code(info="字体: 将顶栏换成 `Fira Code`, 顺便调整下大小") %}
```bash
gsettings set org.gnome.desktop.interface font-name 'Fira Code 11.8'
```
{% end %}

{% code(info="隐藏 `Dash` 栏: 本人是平铺式选手, dash 对我来说无用, 快捷键即可") %}
```bash
...
{% code(info="去掉左上角的 `Activities`: 可将其换成 i3/sway-style, 可安装插件: `space-bar`") %}
```bash
# 自定义工作区的名称, 不然就是默认的数字
...
**注意:**  
隐藏顶栏可以安装 `just-perfection` && `blur-my-shell`, 配置后就是本文 [成品展示](#cheng-pin-zhan-shi) 中的了, 处于 `Overview` 中才显示顶栏  
...
{% code(info="例: 禁用 dash 的快捷键(见上文的置空 `favorite-apps`)") %}
```bash
# Default: Super+"1..9"
for i in $(seq 9)
do
...
{% code(info="例: 修改 `Super+Esc` 为锁屏, 就要先防止冲突") %}
```bash
gsettings set org.gnome.settings-daemon.plugins.media-keys screensaver "['<Super>Escape']" #Default: Super+L
gsettings set org.gnome.mutter.wayland.keybindings restore-shortcuts "[]" # Default: Super+Esc
```
{% end %}
...
{% code(info="vim-style, 改变窗口布局, 替代原有的 `Super+方向键`") %}
```bash
...
{% code(info="对某个窗口 move, resize, kill") %}
```bash
# Move
...
{% code(info="toggle-max, max, min, toggle-fullscreen, show-desktop") %}
```bash
# Toggle max
...
{% code(info="切换工作区, 建议搭配之前消除过渡动画的插件, 流畅感拉满") %}
```bash
for i in $(seq 9)
do
...
{% code(info="覆写 `run-dialog` 的快捷键(默认是 `Alt-F2`)") %}
```bash
...
{% code(info="导出当前的 dconf 数据到某个文件") %}
```bash
dconf dump / > dconf.settings
```
{% end %}

{% code(info="加载某个 dconf 文件到当前系统") %}
```bash
...
{% code(info="在 `~/.config/nixpkgs` 目录下保持如下结构") %}
```txt
nixpkgs/
├── gnome
│   ├── .background
...
上面的 `dconf.nix` 就是刚刚转换得到的 nix 表达式, 在 `home.nix` 中导入它:  

{% code(path="nixpkgs/home.nix") %}
```nix
imports = [
  ./gnome/dconf.nix
];
...
{% code(path="nixpkgs/home.nix" info="对于头像, 在 `home.nix` 添加以下内容") %}
```nix
