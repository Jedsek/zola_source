...
{% code(path="Cargo.toml" hidden=true) %}
```toml
[dependencies]
proc-macro2 = "1.0.78"
quote = "1.0.35"
...
{% code(path="src/lib.rs" hidden=true) %}
```rust
...
{% code(path="tests/test.rs" info="测试中的代码演示如下") %}
```rust
...
{% code(info="输出结果是个类似这样的指针的地址: 0x600cc581a4f0") %}
```rust
fn foo(x: i32) -> i32 {
    x + 1
}
...
{% code(info="对于泛型函数, 我们要手动指定类型, 帮助编译器进行推导") %}
```rust
fn bar<T>(x: T) -> T {
    x
}
...
{% code(info="因此, 我们得用泛型或 `impl` 关键字来表示闭包的实际类型:") %}
```rust
