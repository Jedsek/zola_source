...
## 例子
假设有这么个宏 `map!`(联想一下`vec!`), 能根据 `=>` 判断出 `Key/Value`, 然后生成一个 `HashMap`  
...
对于上面的代码, 最直接的好处就是 c 只是简单的文本替换  
而 rust 则根据上下文推测宏调用的AST节点应被展开为expr类型的AST节点 ~~(相当于自动给你加上了括号)~~  
