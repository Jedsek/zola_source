...
# 原始模式
你可以观看来自 gnu 官网的[说明](https://www.gnu.org/software/libc/manual/html_node/Canonical-or-Not.html), 里面有对此的解释:  
...
{% code(info="cargo run --quiet: 全部输出到屏幕") %}
```txt
This is a str, which will be outputted to stdout
This is a str, which will be outputted to stderr
```
...
{% code(info="cargo run --quiet > output.log: 重定向, 输出到 output.log 中, 默认只重定向 stdout") %}
```txt
This is a str, which will be outputted to stderr
```
{% end %}

{% code(info="cargo run --quiet 1> output.log: 指定文件描述符为 1(stdout), 默认行为") %}
```txt
This is a str, which will be outputted to stderr
```
{% end %}
...
{% code(info="cargo run --quiet 2> error.log: 指定文件描述符为 2(stderr),  stderr 的输出存入 error.log") %}
```txt
This is a str, which will be outputted to stdout
```
{% end %}
...
{% code(info="do {cargo run --quiet} | complete") %}
<pre class="pre-pure">
┌───────────┬──────────────────────────────────────────────────┐
│ <span class="bg-[#282c34] text-[lightgreen]">stdout</span>    │ This is a str, which will be outputted to stdout │
│           │                                                  │
│ <span class="bg-[#282c34] text-[lightgreen]">stderr</span>    │ This is a str, which will be outputted to stderr │
│           │                                                  │
│ <span class="bg-[#282c34] text-[lightgreen]">exit_code</span> │ 0                                                │
└───────────┴──────────────────────────────────────────────────┘
</pre>
{% end %}
...
{% code(info="do {cat non-exist.txt} | complete") %}
<pre class="pre-pure">
┌───────────┬───────────────────────────────────────────────┐
│ <span class="bg-[#282c34] text-[lightgreen]">stdout</span>    │                                               │
│           │                                               │
│ <span class="bg-[#282c34] text-[lightgreen]">stderr</span>    │ cat: non-exist.txt: No such file or directory │
│           │                                               │
│ <span class="bg-[#282c34] text-[lightgreen]">exit_code</span> │ 1                                             │
└───────────┴───────────────────────────────────────────────┘
</pre>
{% end %}
...
{% code(info="cat README.md | wc -m: 当 cat 向 stdout 输出时被管道传走, 屏幕上不显示被传走的内容") %}
```txt
57
```
{% end %}
...
{% code(info="cat REDME.md | wc -m: 当 cat 向 stderr 输出时无法被传递, 屏幕上显示错误消息, 且 wc 的 stdin 为空") %}
```txt
...
{% code(info="cat README.md | ls: 即使 cat 输出至 stdout, 但 ls 不支持 stdin, 屏幕上不显示前者结果, 数据被丢弃了") %}
```txt
