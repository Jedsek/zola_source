...
{% code(info="复制 go/zig 等语言中的 `defer` 语法") %}
```gleam
...
{% code(info="复制 elixir/haskell/scala 等语言中的 `for-comprehension`") %}
```gleam
import gleam/list
...
{% code(info="复制 gleam 自己的 try 表达式(目前已被use取代)") %}
```rust
